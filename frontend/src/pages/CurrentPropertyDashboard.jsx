...
const initiatePayment = async (paymentType) => {
                        ...
                        try {
                            ...
                            if (paymentType === 'deposit') {
                                if (!userBooking) {
                                    ...
                                    await axios.put(
                                        `${import.meta.env.VITE_SERVER_URL}/booking/bookings/${userBooking._id}`,
                                        { paymentStatus: "completed" },
                                        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
                                    );
                                }
                                ...
                                if (unpaidRents.length > 0) {
                                    ...
                                    await axios.put(
                                        `${import.meta.env.VITE_SERVER_URL}/rents/rent/${currentRent._id}`,
                                        {
                                            payment_status: "paid",
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}/review/${propertyId}/review`, {
                user_id: user?.user?._id,
                property_id: propertyId,
                landlord_id: property?.landlord_id?._id,
                comment: propertyComment,
    ...
    const handleSubmitRating = async () => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}/review/${propertyId}/landlord-rating`, {
                user_id: user?.user?._id,
                property_id: propertyId,
                landlord_id: property?.landlord_id?._id,
                trustScore: landlordRating,
