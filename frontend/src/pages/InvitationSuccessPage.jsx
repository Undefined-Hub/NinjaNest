    ...
    const sendRentRequest = async (propertyData, userData) => {
        ...
        try {
            ...
            if (!propertyData.landlord_id) {
                ...
                try {
                    const propertyResponse = await axios.get(`${import.meta.env.VITE_SERVER_URL}/property/${propertyData._id}`, {
                        headers: {
            ...
            const response = await axios.post('${import.meta.env.VITE_SERVER_URL}/request', requestData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            });
    ...
    useEffect(() => {
        ...
        const fetchInvitation = async () => {
            try {
                ...
                const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/invitation/details/${invitationId}`, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                });
                ...
                if (invitationData.status === 'accepted') {
                    ...
                    try {
                        const propertyResponse = await axios.get(`${import.meta.env.VITE_SERVER_URL}/property/${invitationData.propertyId._id}`, {
                            headers: {
                                Authorization: `Bearer ${localStorage.getItem('token')}`
                            }
                        });
